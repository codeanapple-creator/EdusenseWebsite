    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
          <div>
            <div className="text-xs uppercase tracking-[0.22em] font-bold text-[#f97316] mb-1">Parent space</div>
          ...
          <Link to="/behaviourscope">
            <Button className="rounded-full bg-[#0a1f5c] hover:bg-[#142c7a] font-bold px-6 py-3 btn-lift" data-testid="parent-find-niche-btn">
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          <Card className="rounded-3xl p-6 lg:col-span-2 cosmic-bg text-white border-0" data-testid="parent-niche-card">
            <div className="flex items-center justify-between mb-4">
              <div className="text-xs uppercase tracking-[0.22em] font-bold text-orange-300">Latest niche reading</div>
              {history[0] && <Badge className="rounded-full bg-[#0a1f5c]/20 border-orange-400/30 text-orange-200 font-bold">{history[0].sun_sign}</Badge>}
            </div>
            {lastReading ? (
              <>
                <div className="text-3xl md:text-4xl font-black tracking-tight">{lastReading.niche}</div>
                <p className="text-blue-100/80 mt-3 text-sm leading-relaxed">{lastReading.summary}</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  {(lastReading.traits || []).slice(0, 4).map((t, i) => (
                    <Badge key={i} className="rounded-full bg-[#f97316]/20 border border-yellow-400/40 text-[#fdba74] font-bold">{t}</Badge>
                  ))}
                </div>
              </>
            ) : (
              <>
                ...
                <p className="text-blue-100/70 mt-2 text-sm">Begin by entering your child's birth details to discover their niche.</p>
                <Link to="/behaviourscope" className="inline-block mt-4">
                  <Button className="rounded-full bg-[#f97316] hover:bg-[#ea580c] text-white font-bold">Start reading</Button>
                </Link>
              </>
            )}
          </Card>
        ...
        <Card className="rounded-3xl p-7 lg:p-9 bg-white border-slate-100 mb-8" data-testid="recommendations-card">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center">
              <BookOpen className="text-orange-600" size={20} strokeWidth={2.5} />
            </div>
            <div>
          ...
          <div className="grid md:grid-cols-3 gap-4 items-end">
            ...
            <Button onClick={fetchRecs} disabled={loading} className="rounded-full bg-[#f97316] hover:bg-[#ea580c] font-bold py-6 btn-lift" data-testid="rec-generate-btn">
          ...
          {recs && (
            <div className="mt-8 grid lg:grid-cols-3 gap-6" data-testid="recommendations-result">
              <div>
                <div className="text-xs uppercase tracking-widest font-bold text-[#f97316] mb-3 flex items-center gap-1.5"><BookOpen size={14} strokeWidth={2.5} /> Books</div>
                <ul className="space-y-3">
                  {recs.books.map((b, i) => (
                    <li key={i} className="bg-white/60 rounded-2xl p-4 border border-orange-100">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest font-bold text-[#0a1f5c] mb-3 flex items-center gap-1.5"><ExternalLink size={14} strokeWidth={2.5} /> Links</div>
                <ul className="space-y-3">
                  {recs.links.map((l, i) => (
                    <li key={i} className="bg-indigo-50/60 rounded-2xl p-4 border border-indigo-100">
                      <a href={l.url} target="_blank" rel="noopener noreferrer" className="font-bold text-[#0a1f5c] hover:underline inline-flex items-center gap-1.5">{l.title} <ExternalLink size={12} /></a>
              ...
              <div>
                <div className="text-xs uppercase tracking-widest font-bold text-orange-600 mb-3 flex items-center gap-1.5"><Activity size={14} strokeWidth={2.5} /> Activities</div>
                <ul className="space-y-3">
                  {recs.activities.map((a, i) => (
                    <li key={i} className="bg-sky-50/60 rounded-2xl p-4 border border-sky-100">
                      <div className="flex items-center justify-between gap-2">
                        <div className="font-bold text-slate-900">{a.title}</div>
                        <Badge className="rounded-full bg-orange-100 text-[#f97316] border-orange-200 font-bold text-xs"><Clock size={12} className="mr-1" /> {a.duration_minutes}m</Badge>
                      </div>
        ...
        {history.length > 0 && (
          <Card className="rounded-3xl p-7 bg-white border-slate-100" data-testid="parent-history-card">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {history.map((h) => (
                <div key={h.id} className="rounded-2xl border border-slate-200 p-4 bg-white/40">
                  <div className="flex items-center justify-between mb-1">
                    <div className="font-bold text-slate-900">{h.request.name}</div>
                    <Badge className="rounded-full bg-[#0a1f5c]/10 text-[#0a1f5c] border-[#0a1f5c]/20 font-bold text-xs">{h.sun_sign}</Badge>
                  </div>
                  <div className="text-sm font-bold text-[#f97316]">{h.result?.niche}</div>
