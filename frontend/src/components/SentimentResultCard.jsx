...
const SENTIMENT_STYLES = {
  ...
  mixed: { bg: "bg-amber-100", text: "text-amber-700", border: "border-amber-200", Icon: Activity },
};
...
export default function SentimentResultCard({ result, compact = false, testid = "sentiment-result" }) {
  ...
  return (
    <div className={`rounded-2xl border ${style.border} bg-white p-5 ${compact ? "" : "p-6"}`} data-testid={testid}>
      ...
      <div className="mb-5">
        <div className="text-xs uppercase tracking-widest font-bold text-slate-500 mb-3">NRC Emotions (Tiwari, 2024)</div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {nrcEntries.map(([k, v]) => (
            <div key={k} className="rounded-lg border border-slate-200 p-2.5">
              ...
              <Progress value={v * 100} className="h-1.5 [&>div]:bg-indigo-500" />
            </div>
          ))}
        </div>
      </div>
