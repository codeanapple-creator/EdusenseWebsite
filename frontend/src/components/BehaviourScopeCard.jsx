...
export default function BehaviourScopeCard({ scope, testid = "behaviour-scope-card" }) {
  ...
  return (
    <Card className="rounded-3xl p-7 lg:p-10 bg-white border-orange-100 shadow-xl" data-testid={testid}>
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        <div className="rounded-2xl bg-white/60 border border-orange-100 p-4">
          <div className="flex items-center gap-2 text-orange-700 text-xs uppercase tracking-widest font-bold mb-2"><Users size={14} strokeWidth={2.5} /> Social</div>
        ...
        <div className="rounded-2xl bg-sky-50/60 border border-sky-100 p-4">
          <div className="flex items-center gap-2 text-[#f97316] text-xs uppercase tracking-widest font-bold mb-2"><Smile size={14} strokeWidth={2.5} /> Emotional</div>
        ...
        <div className="rounded-2xl bg-indigo-50/60 border border-indigo-100 p-4">
          <div className="flex items-center gap-2 text-[#0a1f5c] text-xs uppercase tracking-widest font-bold mb-2"><Brain size={14} strokeWidth={2.5} /> Learning</div>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {(scope.growth_areas || []).map((s, i) => (
            <Badge key={i} className="rounded-full bg-orange-100 text-orange-700 hover:bg-orange-100 border-orange-200 font-bold px-3 py-1.5">{s}</Badge>
          ))}
        </div>
      </div>

      <div className="rounded-2xl bg-gradient-to-br from-sky-50 to-indigo-50 border border-orange-200/60 p-5">
        <div className="text-xs uppercase tracking-widest font-bold text-[#f97316] mb-3">Parenting tips</div>
